// Je cree le composant d'affichage des resultats de prediction
"use client";
...
export function PredictionResults({
  ...
  useGSAP(
    () => {
      ...
      tl.from(".result-consensus", { y: 30, opacity: 0, duration: 0.6 })
        .from(".result-model-card", { x: -30, opacity: 0, stagger: 0.2, duration: 0.5 }, "-=0.2")
        .from(".result-factors", { y: 20, opacity: 0, duration: 0.5 }, "-=0.2");
    },
    { scope: containerRef }
  );
  ...
  return (
    <div ref={containerRef} className="max-w-4xl mx-auto space-y-8">
      {/* Consensus principal */}
      <div className={`result-consensus glass p-5 sm:p-8 text-center ${riskStyle.bg}`}>
        <p className={clsx("text-sm font-semibold uppercase tracking-wider mb-6", riskStyle.text)}>
          Risque {consensus.risk_level}
        </p>

        <RiskGauge probability={consensus.probability} />

        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 mt-6 text-xs sm:text-sm">
          ...
          <div className="space-y-3">
            {sortedContributions.map(([name, value]) => (
              <div key={name} className="flex items-center gap-2 sm:gap-3">
                ...
                <span className="font-mono text-[10px] sm:text-xs w-12 sm:w-14 text-right">
                  {(value * 100).toFixed(1)}%
                </span>
              </div>
            ))}
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        <button
          onClick={onModify}
          className="glass glass-hover px-6 py-3 rounded-xl text-sm font-medium text-text-secondary"
        >
          Modifier le profil
        </button>
        <button
